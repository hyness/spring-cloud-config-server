...
class FileSystemBackendTest {
    @EnumSource
    @ParameterizedTest
    internal fun `given a config server, when configured with file system backend, is valid`(type: ContainerType) {
    ...
    companion object {
        @Container
        val cloudConfigContainer: DockerComposeContainer<*> = DockerComposeContainer(File("examples/native/docker-compose.yml"))
