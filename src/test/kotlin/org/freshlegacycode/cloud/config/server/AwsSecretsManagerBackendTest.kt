...
class AwsSecretsManagerBackendTest {
    ...
    internal fun populateData() {
        cloudConfigContainer.getContainerByServiceName("localstack")
            .ifPresent {
                it.execInContainer("sh", "/data/populate-secretsmanager.sh")
            }
    }

    @EnumSource
    @ParameterizedTest
    internal fun `given a config server, when configured with aws secrets manager backend, is valid`(type: ContainerType) {
    ...
    companion object {
        @Container
        val cloudConfigContainer: DockerComposeContainer<*> = DockerComposeContainer(File("examples/aws/secretsmanager/docker-compose.yml"))
